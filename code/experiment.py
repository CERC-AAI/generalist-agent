...
def experiment(
    ...
    os.environ['PYTHONHASHSEED'] = str(variant["seed"])
    ...
    if variant["topktraj"]:
        print("Doind topktraj")
        sorted_inds = np.argsort(returns)  # lowest to highest
        print(returns[sorted_inds])
    else:
        np.random.seed(99)
        sorted_inds = np.arange(len(returns))
        np.random.shuffle(sorted_inds)
        np.random.seed(variant["seed"])
    ...
    if log_to_wandb:
        name = str(variant["topktraj"]) + "_" + str(timesteps) + "_" + variant["env"] + "_" + variant["dataset"] +  "_" + str(variant["scratch"]) + "_" + variant["pretrained_lm"] + "_" + str(variant["frozen"]) + "_" + str(variant["gpt_kmeans"]) + "_" + str(variant["share_input_output_proj"])
        wandb.init(
            name=name,
            group=group_name,
            project="scaling-decision-transformer-schedule",
            config=variant,
        )
    ...
    num_iters =  variant["max_iters"]
    if variant["freeze_schedule"]:
        num_iters = 5
...
if __name__ == "__main__":
    ...
    parser.add_argument("--topktraj", action="store_true", default=False)
    ...
    parser.add_argument(
        "--checkpoints_dir",
        default="/home/mila/k/kshitij.gupta/projects/can-wikipedia-help-offline-rl/decoder-only-transformer",
        type=str,
    ...
    parser.add_argument("--freeze_schedule", action="store_true", default=False)
    ...
    parser.add_argument("--frozen_uce_style", action="store_true", default=False)
